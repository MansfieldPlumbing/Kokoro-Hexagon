...
$path = Join-Path $root 'model.ps1'
$tokens = $null; $errors = $null
$ast = [Management.Automation.Language.Parser]::ParseFile($path, [ref]$tokens, [ref]$errors)
if ($errors.Count) { throw 'model.ps1 does not parse' }
...
if (($actual -join "`n") -cne ($expected -join "`n")) { throw 'model.ps1 graph contract changed unexpectedly' }

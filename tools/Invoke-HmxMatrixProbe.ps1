...
# Host runner for staging and executing the Tri-Precision HMX Matrix Contraction Probe on physical Hexagon V73.
...
param(
    ...
    [string]$BuildDirectory = $(
        ...
        Join-Path $buildDir 'hexagon-emission\emitted\KokoroHmxMatrix'
    ),
    [string]$LibraryPath = $(
        $cand = @(
            (Join-Path $BuildDirectory 'libkokoro_hmx_matrix_skel.so'),
            'C:\Dev\Antigravity\Build\Kokoro-QNN\hexagon-emission\emitted\KokoroHmxMatrix\libkokoro_hmx_matrix_skel.so',
            (Join-Path $PSScriptRoot '..\..\..\Build\Kokoro-QNN\hexagon-emission\emitted\KokoroHmxMatrix\libkokoro_hmx_matrix_skel.so')
        ) | Where-Object { Test-Path $_ } | Select-Object -First 1
        if (-not $cand) { $cand = (Join-Path $BuildDirectory 'libkokoro_hmx_matrix_skel.so') }
        $cand
    ),
    [string]$PowerKeeperPath = 'C:\Dev\Antigravity\Build\Kokoro-QNN\vtcm-probe\libkokoro_hmx_power_keeper_skel.so',
...
$harness = Join-Path $PSScriptRoot '..\src\runspace\KokoroHmxMatrixProbe.ps1'
...
$temp = "/data/local/tmp/kokoro-hmx-matrix-$id"
$backup = "files/kokoro-fl/hmx-matrix-backup-$id"
$target = 'files/kokoro-fl/hmx-matrix-emitted'
...
try {
    $files = @(
        @($harness, 'KokoroHmxMatrixProbe.ps1'),
        @($LibraryPath, 'libkokoro_hmx_matrix_skel.so'),
        @($PowerKeeperPath, 'libkokoro_hmx_power_keeper_skel.so')
    )
    ...
    $null = & $run @('shell', "am force-stop $Package && run-as $Package cp $target/KokoroHmxMatrixProbe.ps1 files/Start.ps1 && run-as $Package cp $target/KokoroHmxMatrixProbe.ps1 files/PROFILE.PS1 && monkey -p $Package -c android.intent.category.LAUNCHER 1")
    ...
    $receiptPath = Join-Path $outDir "device-receipt-$id.txt"

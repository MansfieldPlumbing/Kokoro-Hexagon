...
[CmdletBinding()]
param(
    [string]$AssemblyPath = 'C:\Dev\Antigravity\Build\Kokoro-Hexagon\arm64-v8a\managed\by-name\Kokoro-Hexagon.dll'
)
...
if ($graph -cne $source.GraphSHA256) { throw 'Assembly graph hash does not match model.ps1' }
$controlNames = $controls.Split(',', [StringSplitOptions]::RemoveEmptyEntries)
if ($controlNames.Count -ne $source.Controls) { throw 'Assembly control schema does not match model.ps1' }
[pscustomobject]@{
    WindowsLoad = $true

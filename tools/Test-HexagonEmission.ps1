...
$result=& (Join-Path $PSScriptRoot 'Emit-HexagonProbe.ps1') -OutputDirectory $output -Kernel $Kernel
...
& wsl.exe --exec $assembler -triple=hexagon -mcpu=hexagonv73 -filetype=obj "$wslOutput/probe-oracle.s" -o "$wslCheck/oracle.o" 2>$null
...
$rejections=0
foreach($bad in @(@{Op='imm';d=32;i=1},@{Op='imm';d=0;i=32768},@{Op='load';d=0;s=1;Offset=3},@{Op='hi';x=0;i=65536})) {
    try { $null=New-HexagonInstruction $bad 0 0 } catch { $rejections++ }
}
if($rejections -ne 4) { throw 'Encoder accepted an invalid operand' }

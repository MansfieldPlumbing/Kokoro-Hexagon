...
# Host runner for staging and executing the emitted Kokoro R0Sub0 kernel on Hexagon V73.
...
param(
    ...
    [string]$BuildDirectory = $(
        ...
        Join-Path $buildDir 'hexagon-emission\emitted\KokoroR0Sub0'
    ),
...
$harness = Join-Path $PSScriptRoot '..\src\runspace\KokoroR0Sub0Probe.ps1'
...
$library = Join-Path $BuildDirectory 'libkokoro_r0sub0_skel.so'
if (-not (Test-Path $library)) { throw "Emitted library not found: $library" }
$libraryHash = (Get-FileHash $library).Hash

$id = [Guid]::NewGuid().ToString('N')
$temp = "/data/local/tmp/kokoro-r0sub0-$id"
$backup = "files/kokoro-fl/r0sub0-backup-$id"
$target = 'files/kokoro-fl/r0sub0-emitted'
...
try {
    $files = @(
        @($harness, 'KokoroR0Sub0Probe.ps1'),
        @($library, 'libkokoro_r0sub0_skel.so')
    )
    ...
    $null = & $run @('shell', "am force-stop $Package && run-as $Package cp $target/KokoroR0Sub0Probe.ps1 files/Start.ps1 && run-as $Package cp $target/KokoroR0Sub0Probe.ps1 files/PROFILE.PS1 && monkey -p $Package -c android.intent.category.LAUNCHER 1")

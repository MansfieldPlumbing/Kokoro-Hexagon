...
# Host runner for staging and executing the HMX lock probe on physical Hexagon V73.
...
param(
    ...
    [string]$BuildDirectory = $(
        ...
        Join-Path $buildDir 'hexagon-emission\emitted\KokoroHmxLock'
    ),
...
$harness = Join-Path $PSScriptRoot '..\src\runspace\KokoroHmxLockProbe.ps1'
...
$library = Join-Path $BuildDirectory 'libkokoro_hmx_lock_skel.so'
if (-not (Test-Path $library)) { throw "Emitted library not found: $library" }

$id = [Guid]::NewGuid().ToString('N')
$temp = "/data/local/tmp/kokoro-hmx-$id"
$backup = "files/kokoro-fl/hmx-backup-$id"
$target = 'files/kokoro-fl/hmx-lock-emitted'
...
try {
    $files = @(
        @($harness, 'KokoroHmxLockProbe.ps1'),
        @($library, 'libkokoro_hmx_lock_skel.so')
    )
    ...
    $null = & $run @('shell', "am force-stop $Package && run-as $Package cp $target/KokoroHmxLockProbe.ps1 files/Start.ps1 && run-as $Package cp $target/KokoroHmxLockProbe.ps1 files/PROFILE.PS1 && monkey -p $Package -c android.intent.category.LAUNCHER 1")

...
param(
    ...
    # Exact NuGet versions. Empty means the version pinned in
    # lib/pwsh-build-manifest.json.
...
function Get-SetupHelpText {
    @'
Kokoro Hexagon autonomous build pipeline

USAGE
  pwsh -NoProfile -File .\setup-kokoro.ps1 [options]
...
OPTIONS
  ...
  -Admission NativeActivity Build the DEX-free CoreCLR appliance. This fork
                            does not admit the Xamarin/Mono packaging path.
  ...
  -PowerShell <version>     System.Management.Automation and PowerShell SDK.
                            ...
                            version pinned in lib/pwsh-build-manifest.json.
...
EXAMPLES
  .\setup-kokoro.ps1
  .\setup-kokoro.ps1 -h
  .\setup-kokoro.ps1 -c -Step 1
  .\setup-kokoro.ps1 -c -Step 11
  .\setup-kokoro.ps1 -c -Step 1-5
  .\setup-kokoro.ps1 -c -Step 11 -PowerShell 7.7.0-preview.3
  .\setup-kokoro.ps1 -c -Step 11 -DeletePackages
  .\setup-kokoro.ps1 -Interactive
...
function Get-SuggestedUserDirectory {
    ...
    if ($IsWindows) {
        return Join-Path ([Environment]::GetFolderPath('LocalApplicationData')) 'Kokoro-Hexagon'
    }
    $home_ = [Environment]::GetFolderPath('UserProfile')
    if ($IsMacOS) {
        if ($Kind -eq 'Cache') { return Join-Path $home_ 'Library/Caches/Kokoro-Hexagon' }
        return Join-Path $home_ 'Library/Application Support/Kokoro-Hexagon'
    }
    if ($Kind -eq 'Cache') {
        $base = if ($env:XDG_CACHE_HOME) { $env:XDG_CACHE_HOME } else { Join-Path $home_ '.cache' }
        return Join-Path $base 'kokoro-hexagon'
    }
...
function Resolve-WritePlan {
    ...
    if ([string]::IsNullOrWhiteSpace($SigningKeyPath)) {
        ...
        $existingKey = Join-Path $dataRoot 'kokoro-hexagon-signing.pfx'
        $script:SigningKeyPath = if (Test-Path -LiteralPath $existingKey -PathType Leaf) { $existingKey }
                                 else { Join-Path (Join-Path $dataRoot 'Keys') 'kokoro-hexagon-signing.pfx' }
    }
...
function Get-KokoroApplianceDispatch {
    $path = Join-Path $script:RepositoryRoot 'src/appliance/Kokoro.ApplianceExpression.ps1'
    ...
    if ($lambda.ReturnType -ne [int] -or $lambda.Parameters.Count -ne 1 -or $lambda.Parameters[0].Type -ne [string]) {
...
function New-PwshActivityAssemblyBytes {
    ...
    foreach ($name in $loadNames) {
        $candidate = $Candidates |
            ...
            Select-Object -First 1
        if ($null -eq $candidate) { throw "Cannot emit $script:AssemblyName.dll without '$name'." }
    ...
    try {
        ...
        $identity = [Reflection.AssemblyName]::new($script:AssemblyName)
        $builder = [Reflection.Emit.PersistedAssemblyBuilder]::new($identity, [object].Assembly)
        $module = $builder.DefineDynamicModule("$script:AssemblyName.dll")
        ...
        if ($Admission -eq 'NativeActivity') {
            ...
            $try = New-ClrBlock @() @(
                ...
                (New-ClrAssign $resultVar (New-StaticCall $kokoroDispatchMethod @((New-ClrConstant 'status' ([string]))))),
                (& $log $infoPriority (New-StaticCall (Get-ExactMethod ([string]) 'Concat' @([string], [string])) @(
                    (New-ClrConstant 'KOKORO compiled status 0x' ([string])),
            ...
            $run = Add-PersistedMethod $nativeHostType 'Run' ([Reflection.MethodAttributes]'Private,Static,HideBySig') ([int]) @() `
                ([Func[int]]) @() (New-ClrBlock @($issVar, $runspaceVar, $resultVar) @($try))
...
function Add-GeneratedAssemblyCandidates {
    $generated = [ordered]@{
        ...
        "$script:AssemblyName.dll" = New-PwshActivityAssemblyBytes -Candidates $script:BuildContext.PayloadCandidates
    }
...
function Invoke-SelectionStep {
    ...
    $entryAssemblySlots = @($manifest | Where-Object { $_ -ceq 'Pwsh.dll' }).Count
    if ($entryAssemblySlots -ne 1) { throw "The donor assembly order names Pwsh.dll $entryAssemblySlots times; expected exactly once." }
    $manifest = @($manifest | ForEach-Object { if ($_ -ceq 'Pwsh.dll') { "$script:AssemblyName.dll" } else { $_ } })
    ...
    if ($KeepIntermediates) {
        ...
        if (-not (Test-Path -LiteralPath $archiveDirectory -PathType Container)) {
            New-ApprovedDirectory -Path $archiveDirectory
        }
        $archive = [System.Collections.Generic.List[object]]::new()
        foreach ($entry in $selected.GetEnumerator()) {
            $bytes = [byte[]]$entry.Value.Bytes
            $hash = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($bytes))
            Write-BuildFile -Intermediate -Path (Join-Path $archiveDirectory $entry.Key) -Bytes $bytes
            $archive.Add([ordered]@{
                Name = $entry.Key
                Bytes = $bytes.Length
                Sha256 = $hash
                PackageId = [string]$entry.Value.PackageId
                PackagePath = [string]$entry.Value.PackagePath
            })
        }
        $archivePath = Join-Path (Split-Path -Parent $archiveDirectory) 'assemblies.json'
        $archiveJson = ConvertTo-Json -InputObject @($archive) -Depth 4
...
function New-NativeHostLibrary {
    ...
    $data = [ordered]@{
        ...
        domainName   = & $ascii $script:AssemblyName
...
function Invoke-AssembleStep {
    ...
    $outputDirectory = Join-Path $OutputDirectory $script:Target.Abi
    $apkPath = Join-Path $outputDirectory 'Kokoro-Hexagon-unsigned.apk'
    ...
    Write-Host ('[PASS] Step 10 complete: Kokoro-Hexagon-unsigned.apk assembled from {0} entries into {1} bytes, every entry read back byte-identical by an independent reader. SHA-256 {2}' -f
        $report.EntryCount,
        $report.Size,
...
function Get-SigningCertificate {
    ...
    $request = [System.Security.Cryptography.X509Certificates.CertificateRequest]::new(
        'CN=Kokoro Hexagon, O=MansfieldPlumbing, C=US',
        $key,
        [System.Security.Cryptography.HashAlgorithmName]::SHA256,
...
function Invoke-ManifestValidation {
    ...
    try {
        ...
        # Preserve the donor's byte-exact Xamarin fixture as an emitter
        # regression control. The Kokoro product itself admits NativeActivity
        # only and uses the product identity below.
        $xamarin = New-BinaryAxmlManifest -PackageName 'dev.mansfieldplumbing.pwsh' -ActivityClassName 'MainActivity' -ActivityLabel 'Pwsh' -Admission Xamarin
        ...
        Write-Host ('[PASS] Manifest validation: donor Xamarin regression fixture matches; donor and Kokoro NativeActivity manifests pass the reader with {0} and {1} android: attributes matched to public-final.xml ids; {2} malformed-document controls rejected for the expected reason{3}.' -f
